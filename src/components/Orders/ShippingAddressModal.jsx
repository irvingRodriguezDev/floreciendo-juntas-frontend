...
export default function ShippingAddressModal({ open, onClose }) {
  const { AddNewAddress } = useContext(UserContext);
  ...
  // 👉 Cuando se abre el modal, quitar focus previo para evitar warnings
  useEffect(() => {
    if (open) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // 🔒 Deshabilitar el modal para evitar warnings de accesibilidad
  ...
  return (
    <Dialog
      ...
      disableEscapeKeyDown={loading} // Prevent close while loading
      disableRestoreFocus // FIX major focus issues of MUI
      PaperProps={{
        sx: {
          borderRadius: "24px",
